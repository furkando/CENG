...
class Sound:
    ...
    def plot_sound(self, name=""):
        ...
        time_values = np.arange(0, self.number_of_samples,
                                1) / float(self.sampling_rate)
...
def dft(x):
    ...
    n = np.arange(N)
    F_x = np.dot(np.exp(-2j * np.pi * (n.reshape((N, 1)))*n/N), x)
    return F_x
...
def calculate_spectogram(x, window_size=100, stride=50, dft_function=None):
    ...
    i = 0
    S = []
    for i in range((int)((N-window_size)/stride)):
        n = i * stride
        m = n+window_size-1
        S.append(np.fft.fft(x[n:m]))
    return np.array(S)
