...
eater hq bos n x
    | n < (length (hunters x [])) = eater hq (bos ++ (eaterp hq (hq!!n) n (length (hunters x [])) x)) (n+1) x
...
eaterp hq n k m x
    | m < (length (hp x)) = if (snd n)==(snd (hq!!m)) && getPID (preyer (fst ((eaterh (hq) (changer hq) (hq!!m) 0 (k) x)!!0))) /= -1 && getPID (preyer (fst (hq!!m))) /= -1 && (getcatch (hunterer (fst n))-getcatch (hunterer (fst (hq!!k)))) < 2 then eaterp hq (catchH (hunterer (fst n)),snd n) k (m+1) x else eaterp hq n k (m+1) x
    | otherwise = [n]

eaterh pre hq n m k x
    | m < k = if (snd n)==(snd (hq!!m)) &&  (getcatch (hunterer (fst (pre!!m)))-getcatch (hunterer (fst (hq!!m)))) < 2  then [(catchP (preyer (fst n)),snd n)] else eaterh pre hq n (m+1) k x
    | otherwise = [n]
