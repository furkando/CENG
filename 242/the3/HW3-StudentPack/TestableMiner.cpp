...
void TestableMiner::runInternal()
{
    ...
    // minedKoin = new Koin(Utilizer::fetchRandomValue());
    // minedKoin = new Koin(Utilizer::fetchRandomValue());
    // minedKoin = new Koin(Utilizer::fetchRandomValue());
    // minedKoin = new Koin(Utilizer::fetchRandomValue());
    // minedKoin = new Koin(Utilizer::fetchRandomValue());
    ...
    else
    {
        *softForkedBlockchain *= 2;
        ...
        if (!verifyTwoValues(pureBlockchainValue, pureBlockchain->getTotalValue()) || !verifyTwoValues(softForkedBlockchainValue, softForkedBlockchain->getTotalValue()))
        {
            currentGrade -= 2;
        }
    }
