...
struct ext2_inode *find_inode(unsigned int idx)
{
    ...
    struct ext2_group_desc *group_descriptor_block = (struct ext2_group_desc *)(image_map + (EXT2_BLOCK_SIZE + BASE_OFFSET) + (i * sizeof(struct ext2_group_desc)));

    unsigned char *inode_table = (unsigned char *)(image_map + EXT2_BLOCK_SIZE * group_descriptor_block->bg_inode_table);
...
{
    int i = EXT2_ROOT_INO;
    struct ext2_group_desc *blgrp = (struct ext2_group_desc *)(image_map + BASE_OFFSET + EXT2_BLOCK_SIZE);
    unsigned char *itable = (unsigned char *)(image_map + EXT2_BLOCK_SIZE * blgrp->bg_inode_table);
    ...
    while (head != NULL)
    {
        ...
        if (EXT2_S_IFDIR & inode->i_mode)
        {
            ...
            for (j = 0; j < num_blocks; j++)
            {
                ...
                struct ext2_dir_entry *ext2_dir = (struct ext2_dir_entry *)(image_map + EXT2_BLOCK_SIZE * block_no);
...
{
    struct ext2_super_block *super_block = (struct ext2_super_block *)(image_map + BASE_OFFSET);
    struct ext2_group_desc *group_descriptor_block = (struct ext2_group_desc *)(image_map + (BASE_OFFSET + EXT2_BLOCK_SIZE));

    unsigned char *bit_map = (unsigned char *)(image_map + EXT2_BLOCK_SIZE * group_descriptor_block->bg_block_bitmap);
    int size = super_block->s_blocks_per_group / 8;
    int free_block = find_free_block(bit_map, size);
    int i = 1;
    while (free_block < 0)
    {
        group_descriptor_block = (struct ext2_group_desc *)(image_map + (BASE_OFFSET + EXT2_BLOCK_SIZE) + (i * sizeof(struct ext2_group_desc)));
        bit_map = (unsigned char *)(image_map + EXT2_BLOCK_SIZE * group_descriptor_block->bg_block_bitmap);
...
unsigned int new_inode()
{
    ...
    unsigned char *bit_map = (unsigned char *)(image_map + EXT2_BLOCK_SIZE * group_descriptor_block->bg_inode_bitmap);
    ...
    {
        group_descriptor_block = (struct ext2_group_desc *)(image_map + (BASE_OFFSET + EXT2_BLOCK_SIZE) + (i * sizeof(struct ext2_group_desc)));
        bit_map = (unsigned char *)(image_map + EXT2_BLOCK_SIZE * group_descriptor_block->bg_inode_bitmap);
        free_block = find_free_block(bit_map, size);
        if (free_block > 0)
        {
...
int insert_dir_entry(unsigned int target, struct ext2_dir_entry *ext2_dir)
{
    ...
    int size_dir = 8 + ext2_dir->name_len;
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        {
            int size = 0;
            struct ext2_dir_entry *dir_entry = (struct ext2_dir_entry *)(image_map + EXT2_BLOCK_SIZE * copy_inode->i_block[i]);

            while (size < EXT2_BLOCK_SIZE)
            {
                size += dir_entry->rec_len;
                int act_size = 8 + dir_entry->name_len;
        ...
        {
            copy_inode->i_block[i] = new_block();
            copy_inode->i_size++;
            struct ext2_dir_entry *bl = (struct ext2_dir_entry *)(image_map + EXT2_BLOCK_SIZE * copy_inode->i_block[i]);
...
{
    lseek(file_descriptor, group.bg_inode_table * EXT2_BLOCK_SIZE + (index - 1) * sizeof(*inode), SEEK_SET);
...
{
    lseek(file_descriptor, index * EXT2_BLOCK_SIZE + i, SEEK_SET);
...
{
    lseek(file_descriptor, BASE_OFFSET + EXT2_BLOCK_SIZE, SEEK_SET);
...
int main(int argc, char **argv)
{
    ...
    image_map = mmap(NULL, (super.s_blocks_count * EXT2_BLOCK_SIZE), PROT_READ | PROT_WRITE, MAP_SHARED, file_descriptor, 0);
    ...
    if (dest_inode && (dest_inode->i_mode & EXT2_S_IFDIR))
    {
        ...
        if (result > -1)
        {
            struct ext2_inode *copy_inode = find_inode(ext2_dir->inode);
            ...
            while (size < file_size)
            {
                if (i == 12)
                {
                    ...
                    while (size < file_size)
                    {
                            ...
                            while (size < file_size)
                            {
                                if (k == (EXT2_BLOCK_SIZE / 4))
                                {
                                    ...
                                    while (size < file_size)
                                    {
                                        test = new_block();
                                        printf("%d,", test);
                                        memcpy((image_map + EXT2_BLOCK_SIZE * (copy_inode->i_block[i]) + 4 * j), &test, 4);
                                        while (size < file_size)
                                        {
                                            if (k == (EXT2_BLOCK_SIZE / 4))
                                            {
                                            ...
                                            test2 = new_block();
                                            printf("%d,", test2);
                                            memcpy((image_map + EXT2_BLOCK_SIZE * test + 4 * k), &test2, 4);
                                            while (size < file_size)
                                            {
                                                if (l == (EXT2_BLOCK_SIZE / 4))
                                                {
                                                    l = 0;
                                                    k++;
                                                    break;
                                                ...
                                                test3 = new_block();
                                                printf("%d", test3);
                                                if (size + EXT2_BLOCK_SIZE < file_size)
                                                {
                                                    printf(",");
                                                }
                                                memcpy((image_map + EXT2_BLOCK_SIZE * test2 + 4 * l), &test3, 4);
                                                memcpy(image_map + EXT2_BLOCK_SIZE * test3, all_file, EXT2_BLOCK_SIZE);
                                                size += EXT2_BLOCK_SIZE;
                                                all_file = (void *)all_file + EXT2_BLOCK_SIZE;
                                                l++;
                                            }
                                            if (size >= file_size)
                                            {
                                ...
                                test = new_block();
                                printf("%d,", test);
                                memcpy((image_map + EXT2_BLOCK_SIZE * (copy_inode->i_block[i]) + 4 * k), &test, 4);
                                while (size < file_size)
                                {
                                    if (l == (EXT2_BLOCK_SIZE / 4))
                                    {
                                    ...
                                    memcpy((image_map + EXT2_BLOCK_SIZE * test + 4 * l), &test2, 4);
                                    memcpy(image_map + EXT2_BLOCK_SIZE * test2, all_file, EXT2_BLOCK_SIZE);
                                    size += EXT2_BLOCK_SIZE;
                                    all_file = (void *)all_file + EXT2_BLOCK_SIZE;
                                    l++;
                                }
                        ...
                        memcpy((image_map + EXT2_BLOCK_SIZE * (copy_inode->i_block[i]) + 4 * j), &test, 4);
                        memcpy(image_map + EXT2_BLOCK_SIZE * test, all_file, EXT2_BLOCK_SIZE);
                        size += EXT2_BLOCK_SIZE;
                        all_file = (void *)all_file + EXT2_BLOCK_SIZE;
                        j++;
                    }
                ...
                memcpy((image_map + EXT2_BLOCK_SIZE * (copy_inode->i_block[i])), all_file, EXT2_BLOCK_SIZE);
                size += EXT2_BLOCK_SIZE;
                all_file = (void *)all_file + EXT2_BLOCK_SIZE;
                i++;
            }
